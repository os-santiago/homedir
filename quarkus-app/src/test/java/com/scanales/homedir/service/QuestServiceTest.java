...
public class QuestServiceTest {
  ...
  public void testGetQuestBoard_YamlLoading() {
    ...
    Assertions.assertEquals("Crea un fork del repositorio 'homedir' o registra tu propio repositorio de configuración en el directorio de la comunidad.", q1.description());
    Assertions.assertEquals("List Your Shelter", q1.titleEn());
    Assertions.assertEquals("Fork the 'homedir' repository or register your own configuration repository in the community directory.", q1.descriptionEn());
    ...
    Assertions.assertEquals("Únete a la comunidad vinculando tu cuenta de GitHub en tu perfil de HomeDir.", q2.description());
    Assertions.assertEquals("First Contact", q2.titleEn());
    Assertions.assertEquals("Join the community by linking your GitHub account in your HomeDir profile.", q2.descriptionEn());
    ...
    Assertions.assertEquals("Encuentra un bug o sugiere una mejora creando un Issue en el repositorio oficial.", q3.description());
    Assertions.assertEquals("Report an Anomaly", q3.titleEn());
    Assertions.assertEquals("Find a bug or suggest an improvement by creating an Issue in the official repository.", q3.descriptionEn());
    ...
    Assertions.assertEquals("Envía un Pull Request (PR) al repositorio. Puede ser documentación, fix o feature.", q4.description());
    Assertions.assertEquals("Code Contribution", q4.titleEn());
    Assertions.assertEquals("Submit a Pull Request (PR) to the repository. It can be documentation, a fix or a feature.", q4.descriptionEn());
  ...
  public void testQuestTemplateEnglishFallbackToSpanish() {
    ...
    String html = engine.getTemplate("QuestBoardResource/quests")
    ...
    Assertions.assertTrue(html.contains("Spanish Title Test"), "Should fall back to Spanish title when English title is null");
    Assertions.assertTrue(html.contains("English Description Test"), "Should use English description when not null");

    // Test qBlankTitle: should fall back to "Spanish Title Test 2", but use "English Description Test 2"
    Assertions.assertTrue(html.contains("Spanish Title Test 2"), "Should fall back to Spanish title when English title is blank");
    Assertions.assertTrue(html.contains("English Description Test 2"), "Should use English description when not blank");

    // Test qNullDesc: should fall back to "Spanish Description Test 3", but use "English Title Test 3"
    Assertions.assertTrue(html.contains("Spanish Description Test 3"), "Should fall back to Spanish description when English description is null");
    Assertions.assertTrue(html.contains("English Title Test 3"), "Should use English title when not null");

    // Test qBlankDesc: should fall back to "Spanish Description Test 4", but use "English Title Test 4"
    Assertions.assertTrue(html.contains("Spanish Description Test 4"), "Should fall back to Spanish description when English description is blank");
    Assertions.assertTrue(html.contains("English Title Test 4"), "Should use English title when not blank");
  }
