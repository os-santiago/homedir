...
@io.quarkus.runtime.annotations.RegisterForReflection(
    targets = {QuestService.QuestsYaml.class, Quest.class})
public class QuestService {

  private final ObjectMapper yamlMapper = new ObjectMapper(new YAMLFactory());

  /**
   * Progression curve goals: - L1 -> L1000 ~ 100k XP (~5 years with ~55 XP/day average). -
  ...
  @org.eclipse.microprofile.config.inject.ConfigProperty(
      name = "quests.github.repo-owner",
      defaultValue = "os-santiago")
  String repoOwner;

  @org.eclipse.microprofile.config.inject.ConfigProperty(
      name = "quests.github.repo-name",
      defaultValue = "community-directory")
  String repoName;
  ...
  public QuestService() {
    Log.infof(
        ...
        MAX_LEVEL, TARGET_LEVEL, TARGET_LEVEL_XP);
  }
  ...
  public QuestProfile getProfile(String username, Integer historyLimit) {
    ...
    int nextLevelXp =
        currentLevel >= MAX_LEVEL ? getXpForLevel(MAX_LEVEL) : getXpForLevel(currentLevel + 1);
  ...
  private void loadQuests() {
    ...
    try (InputStream is =
        getClass().getClassLoader().getResourceAsStream("quests/initial-quests.yaml")) {
      if (is != null) {
  ...
  public void completeQuest(String userId, String questId) {
    ...
    if (profile.getActiveQuests() == null || !profile.getActiveQuests().contains(questId)) {
      Log.warn(
          "User " + userId + " tried to complete quest " + questId + " without starting it first.");
    ...
    if (!quest.repeatable()) {
      ...
      if (profile.getHistory() != null) {
        ...
        alreadyCompleted =
            profile.getHistory().stream().anyMatch(item -> item.title().equals(expectedTitle));
      }

      if (alreadyCompleted) {
        Log.warn(
            "User " + userId + " tried to complete non-repeatable quest " + questId + " again.");
  ...
  public void fixQuestHistory(String userId) {
    ...
    if (distinctHistory.size() < originalHistory.size()) {
      Log.info(
          "Fixing history for user "
              + userId
              + ". Removed "
              + (originalHistory.size() - distinctHistory.size())
              + " duplicates.");
