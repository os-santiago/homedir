 ...
 *
 * <p>This is the Phase 1 foundation for the Achievement Hub (issue #1323, parent #1043). Real
 * per-user achievement state via the GitHub API is added in a later phase; here every entry is
 * {@link Achievement.Status#LOCKED}.
 */
@ApplicationScoped
public class AchievementCatalog {
  ...
  /** Maps an achievement key to the org repo(s) that help earn it. */
  ...
  private static AchievementGuide guide(
    ...
    return new AchievementGuide(
        Achievement.locked(key, title, description, descriptionEs, category, docUrl),
        List.of(repos));
  }
}
