package com.scanales.homedir.view;

import com.scanales.homedir.model.CharacterProfile;
import com.scanales.homedir.model.UserSession;
import com.scanales.homedir.service.CharacterService;
...
public class GlobalTemplateExtensions {
  ...
  public static SystemStatus system() {
    try {
      com.scanales.homedir.service.PersistenceService persistence =
          Arc.container().instance(com.scanales.homedir.service.PersistenceService.class).get();
      if (persistence != null) {
        boolean lowDisk = persistence.isLowDiskSpace();
        // We could also check queue depth, but low disk is the critical one for now
        return new SystemStatus(
            lowDisk, lowDisk ? "Modo degradado: Almacenamiento crítico." : "Normal");
      }
    } catch (Exception e) {
      // ignore
    }
  ...
  public static com.scanales.homedir.config.AppMessages i18n() {
    try {
      return Arc.container().instance(com.scanales.homedir.config.AppMessages.class).get();
    } catch (Exception e) {
      return null;
    }
  }
