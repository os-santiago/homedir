...
public class LogoutResource {
  ...
  public Response logout(
    ...
    NewCookie clearSession =
        new NewCookie.Builder("q_session")
            .path("/")
            .maxAge(0)
            .httpOnly(true)
            .secure(secure)
            .sameSite(
                "Lax".equalsIgnoreCase(sameSite) ? NewCookie.SameSite.LAX : NewCookie.SameSite.NONE)
            .build();

    // Fallback for q_session (Lax)
    NewCookie clearSessionFallback =
        new NewCookie.Builder("q_session")
            .path("/")
            .maxAge(0)
            .httpOnly(true)
