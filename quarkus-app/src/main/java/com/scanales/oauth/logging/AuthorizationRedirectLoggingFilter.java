...
/**
 * Logs the redirect URL when the user is sent to the OIDC authorization endpoint and any error
 * responses produced during authentication.
 */
@Provider
...
public class AuthorizationRedirectLoggingFilter implements ContainerResponseFilter {
  ...
  public void filter(
    ...
    if (responseContext.getStatus() >= 400) {
      LOG.infov(
          "Authentication error: status={0}, message={1}",
          responseContext.getStatus(), responseContext.getStatusInfo().getReasonPhrase());
    }
  }
}
