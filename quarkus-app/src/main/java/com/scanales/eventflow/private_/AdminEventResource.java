...
public class AdminEventResource {
  ...
  public Response saveTalk(
    ...
    if (event == null) {
      LOG.warnf(
          "accion=charla_crear_validacion_fallida causa=evento_no_encontrado requestId=%s eventoId=%s",
          reqId, eventId);
      return Response.status(Response.Status.NOT_FOUND).build();
    }
